...
//verificar se um número está ou não em uma lista, devolvendo a sua primeira ocorrencia ou 0 em caso negativo
...
//Mesclar 2 listas em uma uma (lembre-se que elas podem ter tamanhoos diferentes)
    //opcao 1
    s_lista * mesclada1(s_lista * l1, s_lista * l2){
        ...
        while(!esta_vazia(l1) && !esta_vazia(l2)){
            ...
            inserir_inicio(remove_inicio(l2), nova);
        }
        while(!esta_vazia(l1)){
            insere_inicio(remove_inicio(l1), nova);
        }
